...
function RecommenderPage({ onSelectStrategy }) {
  ...
  const [accountSz,  setAccountSz]  = useState("$10K–$50K");
  const [experience, setExperience] = useState("Intermediate");
  const [aiResult,   setAiResult]   = useState(null);
  const [mapResult,  setMapResult]  = useState(null);
  const [loading,    setLoading]    = useState(false);
  const [mode,       setMode]       = useState("ai"); // "ai" | "quick"
  const card = { background:C.card, border:"1px solid "+C.border, borderRadius:10, padding:16 };

  // Quick rule-based recommend (instant, no API call)
  const quickRecommend = () => {
    const key = `${view}-${vol}-${horizon}`;
    const found = STRAT_REC_MAP[key] || ["Bull Call Spread","Bear Put Spread","Butterfly"];
    setMapResult(found);
    setAiResult(null);
  };

  // AI-powered recommend via Claude
  const aiRecommend = async () => {
    setLoading(true); setAiResult(null); setMapResult(null);
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type":"application/json", "anthropic-dangerous-direct-browser-access":"true" },
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 900,
          system: `You are an expert options trading strategist. Recommend 3 options strategies based on the user's inputs. 
Respond ONLY with valid JSON — no markdown, no preamble.
Format: {
  "strategies": [
    {
      "name": "strategy name (must be one of: Covered Call, Protective Put, Bull Call Spread, Bear Put Spread, Iron Condor, Straddle, Strangle, Butterfly, Calendar Spread, Cash Secured Put, Collar, Wheel)",
      "rank": 1,
      "match": 92,
      "why": "2-sentence explanation of why this fits their view",
      "setup": "1-sentence setup instruction",
      "maxProfit": "description",
      "maxLoss": "description",
      "idealIV": "High|Low|Moderate",
      "risk": "Low|Medium|High",
      "tip": "1 pro tip for this person's experience level"
    }
  ],
  "marketContext": "2-sentence overall market insight for this view",
  "warning": "1 key risk to be aware of given inputs"
}`,
          messages: [{ role:"user", content:`
Directional view: ${view}
Volatility expectation: ${vol}
Time horizon: ${horizon}
Risk tolerance: ${riskTol}
Account size: ${accountSz}
Experience level: ${experience}
Stock/ticker focus: ${ticker}

Recommend the 3 best options strategies for this trader right now.` }]
        })
      });
      const d = await res.json();
      ...
      setAiResult({ error: "Could not generate AI recommendations. Try the Quick mode instead." });
    }
    setLoading(false);
  };

  const RISK_COLORS = { Low:C.green, Medium:C.yellow, High:C.red };
  const MATCH_COLOR = (m) => m >= 85 ? C.green : m >= 70 ? C.yellow : C.orange;

  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontSize:18, fontWeight:700, marginBottom:3 }}>🎯 Strategy Recommender</div>
          <div style={{ fontSize:11, color:C.dim }}>Tell us your market view — get AI-matched strategies with setup instructions.</div>
        </div>
        <div style={{ display:"flex", gap:6 }}>
          {[["ai","🤖 AI Mode"],["quick","⚡ Quick Mode"]].map(([id,label])=>(
            <button key={id} onClick={()=>setMode(id)}
              style={{ padding:"6px 14px", background:mode===id?"rgba(56,189,248,0.15)":C.card, border:"1px solid "+(mode===id?C.blue:C.border), borderRadius:7, cursor:"pointer", fontSize:10, fontWeight:700, color:mode===id?C.blue:C.dim, fontFamily:"inherit" }}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1.3fr", gap:16 }} className="main-grid-2">
        {/* ── LEFT: Inputs ── */}
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={card}>
            <div style={{ fontSize:9, fontWeight:700, letterSpacing:"0.1em", color:C.blue, marginBottom:14 }}>YOUR MARKET VIEW</div>

            {/* Ticker */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>TICKER / MARKET</div>
              <input value={ticker} onChange={e=>setTicker(e.target.value.toUpperCase())}
                placeholder="e.g. AAPL, SPY, QQQ"
                style={{ width:"100%", boxSizing:"border-box", background:"#080c14", border:"1px solid "+C.border, borderRadius:7, padding:"9px 12px", color:C.text, fontSize:12, outline:"none", fontFamily:"inherit" }}/>
            </div>

            {/* Directional view */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>DIRECTIONAL VIEW</div>
              <div style={{ display:"flex", gap:6 }}>
                {[["Bullish","📈"," Bullish"],["Bearish","📉"," Bearish"],["Neutral","↔"," Neutral"]].map(([v,e,l])=>(
                  <button key={v} onClick={()=>setView(v)}
                    style={{ flex:1, padding:"10px 4px", background:view===v?"rgba(56,189,248,0.15)":"#0f1422", border:"1px solid "+(view===v?C.blue:C.border), borderRadius:7, cursor:"pointer", fontSize:11, fontWeight:700, color:view===v?C.blue:C.dim, fontFamily:"inherit" }}>
                    {e}{l}
                  </button>
                ))}
              </div>
            ...
            {/* Volatility */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>VOLATILITY EXPECTATION</div>
              <div style={{ display:"flex", gap:6 }}>
                {[["High","🔥"],["Low","❄️"],["Neutral","〰"]].map(([v,e])=>(
                  <button key={v} onClick={()=>setVol(v)}
                    style={{ flex:1, padding:"9px 4px", background:vol===v?"rgba(167,139,250,0.15)":"#0f1422", border:"1px solid "+(vol===v?C.purple:C.border), borderRadius:7, cursor:"pointer", fontSize:11, fontWeight:700, color:vol===v?C.purple:C.dim, fontFamily:"inherit" }}>
                    {e} {v}
                  </button>
                ))}
              </div>
            ...
            {/* Time horizon */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>TIME HORIZON</div>
              <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                {["<1 month","1-3 months","3-6 months","6+ months"].map(h=>(
                  <button key={h} onClick={()=>setHorizon(h)}
                    style={{ flex:1, minWidth:80, padding:"8px 4px", background:horizon===h?"rgba(45,212,191,0.15)":"#0f1422", border:"1px solid "+(horizon===h?C.teal:C.border), borderRadius:7, cursor:"pointer", fontSize:10, fontWeight:700, color:horizon===h?C.teal:C.dim, fontFamily:"inherit" }}>
                    {h}
                  </button>
                ))}
              </div>
            ...
            {mode === "ai" && (<>
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>RISK TOLERANCE</div>
                <div style={{ display:"flex", gap:6 }}>
                  {["Conservative","Moderate","Aggressive"].map(r=>(
                    <button key={r} onClick={()=>setRiskTol(r)}
                      style={{ flex:1, padding:"8px 4px", background:riskTol===r?"rgba(251,191,36,0.15)":"#0f1422", border:"1px solid "+(riskTol===r?C.yellow:C.border), borderRadius:7, cursor:"pointer", fontSize:10, fontWeight:700, color:riskTol===r?C.yellow:C.dim, fontFamily:"inherit" }}>
                      {r}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
                <div>
                  <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>ACCOUNT SIZE</div>
                  <select value={accountSz} onChange={e=>setAccountSz(e.target.value)}
                    style={{ width:"100%", background:"#080c14", border:"1px solid "+C.border, borderRadius:7, padding:"9px 10px", color:C.text, fontSize:11, outline:"none", fontFamily:"inherit" }}>
                    {["<$5K","$5K–$10K","$10K–$50K","$50K–$200K",">$200K"].map(s=><option key={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <div style={{ fontSize:10, color:C.dim, fontWeight:700, marginBottom:6 }}>EXPERIENCE</div>
            ...
            <button onClick={mode==="ai"?aiRecommend:quickRecommend} disabled={loading}
              style={{ width:"100%", padding:"13px", background:loading?"#1a2535":"linear-gradient(135deg,#0369a1,#0ea5e9)", color:loading?C.dim:"#fff", border:"none", borderRadius:8, cursor:loading?"default":"pointer", fontWeight:700, fontSize:13, fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              {loading ? (<><span style={{ width:14, height:14, border:"2px solid #fff3", borderTop:"2px solid #fff", borderRadius:"50%", display:"inline-block", animation:"spin .7s linear infinite" }}/>Analyzing your view…</>) : mode==="ai" ? "🤖 Get AI Recommendations →" : "⚡ Get Quick Recommendations →"}
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          {aiResult && !aiResult.error && (
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {/* Market context */}
              <div style={{ ...card, background:"#060d16", border:"1px solid "+C.blue+"44" }}>
                <div style={{ fontSize:9, color:C.blue, fontWeight:700, letterSpacing:"0.1em", marginBottom:6 }}>🤖 AI MARKET CONTEXT</div>
                <div style={{ fontSize:12, color:"#94a3b8", lineHeight:1.7, marginBottom:8 }}>{aiResult.marketContext}</div>
                {aiResult.warning && (
                  <div style={{ fontSize:11, color:C.yellow, background:"rgba(251,191,36,0.08)", border:"1px solid "+C.yellow+"33", borderRadius:7, padding:"8px 12px" }}>
                    ⚠️ {aiResult.warning}
                  </div>
                )}
              </div>
              ...
              {aiResult.strategies?.map((s, i) => (
                <div key={i} style={{ ...card, borderLeft:"3px solid "+(i===0?C.green:i===1?C.blue:C.purple) }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                    <div>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                        <span style={{ fontSize:16, fontWeight:700, color:C.text }}>#{s.rank} {s.name}</span>
                        <Badge bg={RISK_COLORS[s.risk]+"22"} color={RISK_COLORS[s.risk]}>{s.risk} Risk</Badge>
                      </div>
                    </div>
                    {/* Match score */}
                    <div style={{ textAlign:"center", flexShrink:0 }}>
                      <div style={{ fontSize:22, fontWeight:700, color:MATCH_COLOR(s.match) }}>{s.match}%</div>
                      <div style={{ fontSize:8, color:C.dim }}>match</div>
                    </div>
                  </div>

                  {/* Match bar */}
                  <div style={{ height:4, background:"#1a2535", borderRadius:2, marginBottom:10, overflow:"hidden" }}>
                    <div style={{ width:s.match+"%", height:"100%", background:MATCH_COLOR(s.match), borderRadius:2, transition:"width .5s" }}/>
                  </div>

                  <div style={{ fontSize:12, color:"#94a3b8", lineHeight:1.6, marginBottom:10 }}>{s.why}</div>

                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
                    {[["Setup", s.setup, C.text],["Max Profit", s.maxProfit, C.green],["Max Loss", s.maxLoss, C.red],["Ideal IV", s.idealIV, C.yellow]].map(([l,v,c])=>(
                      <div key={l} style={{ background:"#080c14", borderRadius:6, padding:"8px 10px" }}>
                        <div style={{ fontSize:8, color:C.muted, marginBottom:2 }}>{l}</div>
                        <div style={{ fontSize:10, color:c, fontWeight:700, lineHeight:1.4 }}>{v}</div>
                      </div>
                    ))}
                  </div>

                  {s.tip && (
                    <div style={{ fontSize:10, color:C.teal, background:"rgba(45,212,191,0.08)", borderRadius:6, padding:"8px 10px", marginBottom:10, lineHeight:1.5 }}>
                      💡 Pro tip: {s.tip}
                    </div>
                  )}

                  <button onClick={() => onSelectStrategy(s.name)}
                    style={{ width:"100%", padding:"9px", background:"rgba(56,189,248,0.1)", color:C.blue, border:"1px solid "+C.blue+"44", borderRadius:7, cursor:"pointer", fontWeight:700, fontSize:11, fontFamily:"inherit" }}>
                    Build {s.name} →
                  </button>
                </div>
              ))}
          ...
          {/* AI Error */}
          {aiResult?.error && (
            <div style={{ ...card, border:"1px solid "+C.red+"44", background:"#0d0608" }}>
              <div style={{ color:C.red, fontWeight:700, marginBottom:6 }}>⚠️ Error</div>
              <div style={{ fontSize:11, color:C.dim }}>{aiResult.error}</div>
            </div>
          )}
          ...
          {mapResult && (
            <div style={card}>
              <div style={{ fontSize:9, fontWeight:700, letterSpacing:"0.1em", color:C.blue, marginBottom:12 }}>⚡ QUICK RECOMMENDATIONS</div>
              <div style={{ fontSize:11, color:C.dim, marginBottom:14 }}>
                For a <strong style={{ color:C.blue }}>{view}</strong> view with <strong style={{ color:C.purple }}>{vol} vol</strong>, {horizon}:
              </div>
              {mapResult.map((name, i) => {
                const s = STRATS[name];
                return s ? (
                  <div key={name} style={{ background:"#0f1422", border:"1px solid "+C.border, borderRadius:8, padding:14, marginBottom:10 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                      <span style={{ fontWeight:700, fontSize:13 }}>#{i+1} {name}</span>
                      <Badge bg={s.risk==="Low"?"#052e16":s.risk==="Medium"?"#1c1a0a":"#3b0000"} color={s.risk==="Low"?C.green:s.risk==="Medium"?C.yellow:C.red}>{s.risk} Risk</Badge>
                    </div>
                    <div style={{ fontSize:11, color:C.dim, marginBottom:8, lineHeight:1.5 }}>{s.desc}</div>
                    <button onClick={() => onSelectStrategy(name)}
                      style={{ padding:"6px 16px", background:"#0f2a40", color:C.blue, border:"1px solid "+C.blue+"44", borderRadius:6, cursor:"pointer", fontSize:10, fontWeight:700, fontFamily:"inherit" }}>
                      Build Strategy →
                    </button>
                  </div>
                ) : null;
              })}
              <div style={{ marginTop:10, fontSize:10, color:C.dim, textAlign:"center" }}>
                Switch to 🤖 AI Mode for personalised analysis with setup instructions
              </div>
            </div>
          )}

          {/* Empty state */}
          {!aiResult && !mapResult && !loading && (
            <div style={{ ...card, display:"flex", alignItems:"center", justifyContent:"center", minHeight:320 }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:48, marginBottom:12 }}>🎯</div>
                <div style={{ fontSize:14, fontWeight:700, color:C.dim, marginBottom:6 }}>Set your market view</div>
                <div style={{ fontSize:11, color:C.muted }}>Fill in your view on the left and get matched strategies — AI mode gives personalised setup instructions.</div>
              </div>
            </div>
          )}
        </div>
...
function CommandPalette({ quotes, onSelectTicker, onNavigate, onClose }) {
  ...
  const PAGES = [
    ...
    { id:"news",       label:"Market News",           emoji:"📰", desc:"Real-time news + AV sentiment scoring" },
...
// ── MARKET NEWS PAGE ─────────────────────────────────────────────────────────
// Real-time news via Finnhub (company + general market news)
// Sentiment scoring via Alpha Vantage NEWS_SENTIMENT endpoint
// Auto-refreshes every 2 minutes. Streams ticker-specific news on demand.

const NEWS_CATEGORIES = [
  { id:"general",  label:"🌐 Market",   finnhubCat:"general" },
  { id:"forex",    label:"💱 Forex",    finnhubCat:"forex" },
  { id:"crypto",   label:"₿ Crypto",   finnhubCat:"crypto" },
  { id:"merger",   label:"🤝 M&A",      finnhubCat:"merger" },
];

const SENTIMENT_COLOR = (s) => {
  if (!s && s !== 0) return C.muted;
  if (s >= 0.35)  return C.green;
  if (s >= 0.15)  return "#86efac";
  if (s <= -0.35) return C.red;
  if (s <= -0.15) return "#fca5a5";
  return C.yellow;
};
const SENTIMENT_LABEL = (s) => {
  if (!s && s !== 0) return "—";
  if (s >= 0.35)  return "Bullish";
  if (s >= 0.15)  return "Slightly Bullish";
  if (s <= -0.35) return "Bearish";
  if (s <= -0.15) return "Slightly Bearish";
  return "Neutral";
};
...
function NewsCard({ item, onTickerClick }) {
  const [expanded, setExpanded] = useState(false);
  const sentScore = item.sentiment?.score;
  const sentLabel = SENTIMENT_LABEL(sentScore);
  const sentCol   = SENTIMENT_COLOR(sentScore);
  ...
  return (
    <div style={{
      ...
      borderLeft: "3px solid " + (sentScore >= 0.15 ? C.green : sentScore <= -0.15 ? C.red : C.border)
    }}>
      {/* Image row */}
      {hasImage && (
        <div style={{ height: 140, overflow: "hidden", background: "#0a0e18" }}>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:8, flexWrap:"wrap" }}>
          <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
            <span style={{ fontSize:9, fontWeight:700, color:C.blue, background:"rgba(56,189,248,0.1)", padding:"2px 8px", borderRadius:4, border:"1px solid "+C.blue+"33" }}>
              {item.source}
            </span>
            {item.category && (
              <span style={{ fontSize:9, color:C.muted, padding:"2px 6px", background:"#0f1422", borderRadius:4 }}>
                {item.category}
              </span>
            )}
            {sentScore !== undefined && (
              <span style={{ fontSize:9, fontWeight:700, color:sentCol, background:sentCol+"18", padding:"2px 8px", borderRadius:4, border:"1px solid "+sentCol+"33" }}>
                {sentLabel}
              </span>
            )}
          </div>
          <span style={{ fontSize:9, color:C.muted, flexShrink:0 }}>{timeAgo(item.datetime)}</span>
        </div>

        {/* Headline */}
        <a href={item.url} target="_blank" rel="noopener noreferrer"
          style={{ fontSize:13, fontWeight:700, color:C.text, textDecoration:"none", lineHeight:1.5, display:"block", marginBottom:8 }}>
          {item.headline}
        </a>
        ...
        {item.summary && (
          <div style={{ fontSize:11, color:"#94a3b8", lineHeight:1.7, marginBottom:10 }}>
            {expanded ? item.summary : item.summary.slice(0, 180) + (item.summary.length > 180 ? "…" : "")}
            {item.summary.length > 180 && (
              <span onClick={() => setExpanded(e => !e)}
                style={{ color:C.blue, cursor:"pointer", marginLeft:4, fontSize:10 }}>
        ...
        {item.related && item.related.length > 0 && (
          <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
            <span style={{ fontSize:9, color:C.muted }}>Related:</span>
            {item.related.slice(0, 6).map(t => (
              <span key={t} onClick={() => onTickerClick(t)}
                style={{ fontSize:9, fontWeight:700, color:C.blue, cursor:"pointer", padding:"1px 6px", background:"rgba(56,189,248,0.1)", borderRadius:4, border:"1px solid "+C.blue+"22" }}>
                {t}
              </span>
            ))}
          </div>
        ...
        {sentScore !== undefined && (
          <div style={{ marginTop:10 }}>
            <div style={{ height:3, background:"#1a2535", borderRadius:2, overflow:"hidden" }}>
              <div style={{
                height:"100%", borderRadius:2,
                width: Math.abs(sentScore * 100) + "%",
                marginLeft: sentScore < 0 ? (100 - Math.abs(sentScore * 100)) + "%" : 0,
                background: sentCol, transition:"width .3s"
              }}/>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:3, fontSize:8, color:C.muted }}>
              <span>Bearish</span>
              <span style={{ color:sentCol, fontWeight:700 }}>{sentScore !== undefined ? (sentScore > 0 ? "+" : "") + (sentScore * 100).toFixed(0) : ""}</span>
              <span>Bullish</span>
            </div>
          </div>
        )}
        ...
        <div style={{ marginTop:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <a href={item.url} target="_blank" rel="noopener noreferrer"
            style={{ fontSize:10, color:C.blue, textDecoration:"none", padding:"4px 10px", background:"rgba(56,189,248,0.1)", borderRadius:5, border:"1px solid "+C.blue+"33" }}>
            Read full article →
          </a>
          <span style={{ fontSize:8, color:C.muted }}>via {item.source}</span>
        </div>
      </div>
    </div>
  );
...
function MarketNewsPage({ ticker, onSelectTicker }) {
  const [news, setNews]             = useState([]);
  ...
  const [activeTab, setActiveTab]   = useState("market"); // "market" | "ticker" | "sentiment"
  ...
  // ── Fetch general/category market news from Finnhub ──
  const fetchMarketNews = async (cat = "general") => {
    setLoading(true); setError(null);
    try {
      const r = await fetch(
        `https://finnhub.io/api/v1/news?category=${cat}&minId=0&token=${DATA_CONFIG.FINNHUB_KEY}`
      );
      if (!r.ok) throw new Error(r.status === 429 ? "Rate limited — try again in 60s" : `Error ${r.status}`);
      const d = await r.json();
      const items = Array.isArray(d) ? d.slice(0, 30).map(n => ({
        id:       n.id,
        headline: n.headline,
        summary:  n.summary,
        source:   n.source,
        ...
        image:    n.image,
        datetime: n.datetime,
        related:  n.related ? n.related.split(",").map(s=>s.trim()).filter(Boolean) : [],
        category: cat,
        sentiment: undefined, // filled in by AV if available
      })) : [];
      setNews(items);
  ...
  const fetchTickerNews = async (sym) => {
    if (!sym) return;
    setLoading(true); setError(null);
    const now   = new Date();
    const from  = new Date(now - 7 * 864e5).toISOString().slice(0,10); // 7 days back
    const to    = now.toISOString().slice(0,10);
    try {
      const r = await fetch(
        `https://finnhub.io/api/v1/company-news?symbol=${sym.toUpperCase()}&from=${from}&to=${to}&token=${DATA_CONFIG.FINNHUB_KEY}`
      );
      if (!r.ok) throw new Error(r.status === 429 ? "Rate limited — try again in 60s" : `Error ${r.status}`);
      const d = await r.json();
      const items = Array.isArray(d) ? d.slice(0, 25).map(n => ({
        id:       n.id,
        headline: n.headline,
        summary:  n.summary,
        source:   n.source,
  ...
  const fetchAVSentiment = async (sym) => {
    setAvLoading(true);
    try {
      const tickers = sym ? `&tickers=${sym.toUpperCase()}` : "";
      const r = await fetch(
        `https://www.alphavantage.co/query?function=NEWS_SENTIMENT${tickers}&limit=20&apikey=${DATA_CONFIG.ALPHAVANTAGE_KEY}`
      );
      const d = await r.json();
      if (d.feed && Array.isArray(d.feed)) {
        const items = d.feed.slice(0, 20).map((n, i) => {
          const tickerSentiments = n.ticker_sentiment || [];
          const relevantScore = sym
            ? tickerSentiments.find(t => t.ticker === sym.toUpperCase())?.ticker_sentiment_score
          ...
          return {
            ...
            related:  tickerSentiments.slice(0,5).map(t => t.ticker),
            category: n.topics?.[0]?.topic || "general",
            sentiment: {
              score: parseFloat(relevantScore) || 0,
      ...
      console.warn("[AV News]", e.message);
    }
    setAvLoading(false);
  };
  ...
  useEffect(() => {
    if (!autoRefresh) { clearInterval(countdownRef.current); clearTimeout(refreshTimerRef.current); return; }
    countdownRef.current = setInterval(() => {
      setRefreshIn(s => {
        if (s <= 1) {
          if (activeTab === "ticker" && tickerFilter) {
            fetchTickerNews(tickerFilter);
          } else {
  ...
  }, [autoRefresh, activeTab, category, tickerFilter]);
  ...
  const handleTabChange = (tab) => {
    ...
    setNews([]);
    if (tab === "ticker") {
      const sym = tickerFilter || ticker;
      setTickerFilter(sym);
      fetchTickerNews(sym);
      fetchAVSentiment(sym);
  ...
  const handleTickerSearch = (sym) => {
    if (!sym.trim()) return;
    setTickerFilter(sym.trim().toUpperCase());
    fetchTickerNews(sym.trim().toUpperCase());
    fetchAVSentiment(sym.trim().toUpperCase());
  };

  const displayNews = activeTab === "sentiment" ? avNews : news;
  const card = { background:C.card, border:"1px solid "+C.border, borderRadius:10, padding:16 };
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontSize:18, fontWeight:700, marginBottom:3 }}>📰 Market News</div>
          <div style={{ fontSize:11, color:C.dim }}>
            Real-time news · Finnhub
            {DATA_CONFIG.ALPHAVANTAGE_KEY ? " + Alpha Vantage sentiment" : ""}
            {lastUpdated && <span style={{ marginLeft:8, color:C.muted }}>Updated {lastUpdated.toLocaleTimeString()}</span>}
          </div>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          {/* Auto refresh countdown */}
          <div style={{ display:"flex", alignItems:"center", gap:6, padding:"5px 10px", background:"#0a0e18", borderRadius:7, border:"1px solid "+C.border }}>
            <span style={{ width:6, height:6, borderRadius:"50%", background:autoRefresh?C.green:C.muted, display:"inline-block", animation:autoRefresh?"pulse 2s infinite":"none" }}/>
            <span style={{ fontSize:9, color:autoRefresh?C.green:C.muted, fontWeight:700 }}>
              {autoRefresh ? `Auto-refresh ${refreshIn}s` : "Paused"}
            </span>
            <button onClick={() => setAutoRefresh(a => !a)}
              style={{ fontSize:8, padding:"2px 6px", background:"transparent", color:C.dim, border:"none", cursor:"pointer", fontFamily:"inherit" }}>
              {autoRefresh ? "Pause" : "Resume"}
            </button>
          </div>
          {/* Manual refresh */}
          <button onClick={() => { if (activeTab==="ticker") fetchTickerNews(tickerFilter); else if (activeTab==="sentiment") fetchAVSentiment(tickerFilter); else fetchMarketNews(category); }}
            style={{ padding:"6px 14px", background:"rgba(56,189,248,0.1)", color:C.blue, border:"1px solid "+C.blue+"33", borderRadius:7, cursor:"pointer", fontSize:10, fontWeight:700, fontFamily:"inherit" }}>
            🔄 Refresh
          </button>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:14, borderBottom:"1px solid "+C.border }}>
        {[
          ...
          <button key={id} onClick={() => handleTabChange(id)}
            style={{ padding:"10px 18px", background:"transparent", border:"none", borderBottom:"2px solid "+(activeTab===id?C.blue:"transparent"), color:activeTab===id?C.blue:C.dim, cursor:"pointer", fontWeight:700, fontSize:12, fontFamily:"inherit", transition:"all .15s" }}>
            {label}
          </button>
        ))}
      </div>

      {/* ── MARKET TAB: Category filter ── */}
      {activeTab === "market" && (
        <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap" }}>
          {NEWS_CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => handleCategoryChange(cat.finnhubCat)}
              style={{ padding:"6px 14px", background:category===cat.finnhubCat?"rgba(56,189,248,0.15)":C.card, border:"1px solid "+(category===cat.finnhubCat?C.blue:C.border), borderRadius:20, cursor:"pointer", fontSize:10, fontWeight:700, color:category===cat.finnhubCat?C.blue:C.dim, fontFamily:"inherit" }}>
              {cat.label}
            </button>
          ))}
        </div>
      ...
      {/* ── TICKER TAB: Symbol search ── */}
      {(activeTab === "ticker" || activeTab === "sentiment") && (
        <div style={{ display:"flex", gap:8, marginBottom:14 }}>
          <input
            value={tickerFilter}
            onChange={e => setTickerFilter(e.target.value.toUpperCase())}
            onKeyDown={e => e.key === "Enter" && handleTickerSearch(tickerFilter)}
            placeholder="Enter ticker (e.g. AAPL, TSLA, NVDA)…"
            style={{ flex:1, background:"#080c14", border:"1px solid "+C.border, borderRadius:7, padding:"10px 14px", color:C.text, fontSize:13, outline:"none", fontFamily:"inherit" }}
          />
          <button onClick={() => handleTickerSearch(tickerFilter)}
            style={{ padding:"10px 20px", background:"linear-gradient(135deg,#0369a1,#0ea5e9)", color:"#fff", border:"none", borderRadius:7, cursor:"pointer", fontWeight:700, fontSize:12, fontFamily:"inherit" }}>
            Search
          </button>
        </div>
      ...
      {activeTab === "ticker" && (
        <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap" }}>
          {["AAPL","NVDA","TSLA","META","AMZN","MSFT","GOOGL","AMD","SPY","QQQ"].map(sym => (
            <button key={sym} onClick={() => { setTickerFilter(sym); fetchTickerNews(sym); fetchAVSentiment(sym); }}
              style={{ padding:"5px 12px", background:tickerFilter===sym?"rgba(56,189,248,0.15)":C.card, border:"1px solid "+(tickerFilter===sym?C.blue:C.border), borderRadius:20, cursor:"pointer", fontSize:10, fontWeight:700, color:tickerFilter===sym?C.blue:C.dim, fontFamily:"inherit" }}>
              {sym}
            </button>
          ))}
        </div>
      ...
      {/* ── AV Sentiment summary banner ── */}
      {activeTab === "sentiment" && avNews.length > 0 && (() => {
        const scores = avNews.map(n => n.sentiment?.score || 0).filter(s => s !== 0);
        const avg    = scores.length ? scores.reduce((a,b) => a+b, 0) / scores.length : 0;
        const bulls  = scores.filter(s => s >= 0.15).length;
        const bears  = scores.filter(s => s <= -0.15).length;
        const neutral= scores.length - bulls - bears;
        return (
          <div style={{ ...card, marginBottom:14, background:"#070a12", border:"1px solid "+SENTIMENT_COLOR(avg)+"44" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontSize:9, color:C.muted, fontWeight:700, letterSpacing:"0.1em", marginBottom:4 }}>OVERALL MARKET SENTIMENT · ALPHA VANTAGE</div>
                <div style={{ fontSize:22, fontWeight:700, color:SENTIMENT_COLOR(avg) }}>
                  {SENTIMENT_LABEL(avg)}
                  <span style={{ fontSize:12, marginLeft:8, color:C.dim }}>({avg > 0 ? "+" : ""}{(avg * 100).toFixed(0)} avg score)</span>
                </div>
              </div>
              <div style={{ display:"flex", gap:16 }}>
                {[["Bullish", bulls, C.green],["Neutral", neutral, C.yellow],["Bearish", bears, C.red]].map(([l,v,c]) => (
                  <div key={l} style={{ textAlign:"center" }}>
            ...
            {/* Sentiment bar */}
            <div style={{ marginTop:12, height:6, background:"#1a2535", borderRadius:3, overflow:"hidden", display:"flex" }}>
              <div style={{ width: (bears/scores.length*100||0)+"%" , background:C.red,    transition:"width .5s" }}/>
              <div style={{ width: (neutral/scores.length*100||0)+"%", background:C.yellow, transition:"width .5s" }}/>
              <div style={{ width: (bulls/scores.length*100||0)+"%" , background:C.green,  transition:"width .5s" }}/>
            </div>
            <div style={{ fontSize:8, color:C.muted, marginTop:4 }}>Based on {scores.length} articles · Alpha Vantage News Sentiment API</div>
          </div>
        );
      })()}
      ...
      {(loading || avLoading) && (
        <div style={{ ...card, textAlign:"center", padding:"40px 0" }}>
          <div style={{ display:"flex", gap:8, justifyContent:"center", marginBottom:12 }}>
            {[0,1,2].map(i => (
              <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:C.blue, animation:`pulse ${0.5+i*0.15}s ease infinite alternate` }}/>
            ))}
          </div>
          <div style={{ fontSize:12, color:C.dim }}>
            {loading ? "Fetching latest news from Finnhub…" : "Loading Alpha Vantage sentiment…"}
          </div>
        </div>
      )}
      ...
      {/* ── Error ── */}
      {error && !loading && (
        <div style={{ ...card, border:"1px solid "+C.red+"44", background:"#1a0608", marginBottom:14 }}>
          <div style={{ fontSize:13, color:C.red, fontWeight:700, marginBottom:6 }}>⚠️ {error}</div>
          <div style={{ fontSize:11, color:C.dim, marginBottom:10 }}>
            {error.includes("429") || error.includes("Rate")
              ? "Finnhub free tier: 60 API calls/minute. Wait a moment then refresh."
              : "Check your Finnhub API key in DATA_CONFIG, or try again."}
          </div>
          <button onClick={() => fetchMarketNews(category)}
            style={{ padding:"6px 16px", background:"rgba(248,113,113,0.15)", color:C.red, border:"1px solid "+C.red+"44", borderRadius:7, cursor:"pointer", fontSize:10, fontWeight:700, fontFamily:"inherit" }}>
            Try Again
          </button>
        </div>
      )}
      ...
      {!loading && !error && displayNews.length > 0 && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:14 }} className="main-grid-2">
          {displayNews.map((item, i) => (
            <NewsCard key={item.id || i} item={item} onTickerClick={(sym) => {
              onSelectTicker(sym);
              setTickerFilter(sym);
              handleTabChange("ticker");
            }} />
          ))}
        </div>
      )}
      ...
      {!loading && !error && displayNews.length === 0 && (
        <div style={{ ...card, textAlign:"center", padding:"60px 0" }}>
          ...
          <div style={{ fontSize:14, color:C.dim, marginBottom:6 }}>
            {activeTab === "ticker" ? `No news found for ${tickerFilter}` : "No news available"}
          </div>
          <div style={{ fontSize:11, color:C.muted }}>
            {activeTab === "ticker" ? "Try a different ticker or broader date range" : "Try refreshing or switching category"}
          </div>
        </div>
      )}

      {/* ── API attribution ── */}
      <div style={{ marginTop:24, padding:"10px 14px", background:"#060810", borderRadius:8, border:"1px solid "+C.border, fontSize:9, color:C.muted, display:"flex", gap:16, flexWrap:"wrap" }}>
        <span>📡 News: <strong style={{ color:C.blue }}>Finnhub.io</strong> (real-time)</span>
        <span>📊 Sentiment: <strong style={{ color:C.purple }}>Alpha Vantage</strong> (NLP-scored)</span>
        <span style={{ marginLeft:"auto" }}>Not financial advice · Always verify before trading</span>
      </div>
    </div>
  );
}

// ── MARKET NEWS STRIP (used on market page) ──────────────────────────────────
function MarketNewsStrip({ onSelectTicker, onViewAll }) {
  const [stripNews,    setStripNews]    = useState([]);
  const [stripLoading, setStripLoading] = useState(true);

  useEffect(() => {
    if (!DATA_CONFIG.USE_LIVE_DATA || !DATA_CONFIG.FINNHUB_KEY) {
      setStripLoading(false);
      return;
    }
    fetch(`https://finnhub.io/api/v1/news?category=general&minId=0&token=${DATA_CONFIG.FINNHUB_KEY}`)
      .then(r => r.ok ? r.json() : [])
      .then(d => { if (Array.isArray(d)) setStripNews(d.slice(0, 5)); setStripLoading(false); })
  ...
  if (stripLoading) return (
    <div style={{...card, marginBottom:14, display:"flex", alignItems:"center", gap:10, padding:"10px 16px"}}>
      <span style={{fontSize:9, color:C.blue, fontWeight:700, flexShrink:0, animation:"pulse 1.5s infinite"}}>📰 LIVE NEWS</span>
      <span style={{fontSize:10, color:C.muted}}>Loading latest headlines…</span>
    </div>
  );

  if (!stripNews.length) return null;
    ...
    <div style={{...card, marginBottom:14, padding:"10px 16px"}}>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8}}>
        <div style={{display:"flex", alignItems:"center", gap:6}}>
          ...
          <span style={{fontSize:9, fontWeight:700, color:C.blue, letterSpacing:"0.1em"}}>LIVE NEWS · FINNHUB</span>
        </div>
        <button onClick={onViewAll}
          style={{fontSize:9, color:C.blue, background:"rgba(56,189,248,0.1)", border:"1px solid "+C.blue+"33", borderRadius:5, padding:"3px 10px", cursor:"pointer", fontFamily:"inherit"}}>
          View all 📰 →
        </button>
      </div>
      <div style={{display:"flex", flexDirection:"column", gap:0}}>
        {stripNews.map((n, i) => (
          <div key={i} style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12, padding:"7px 0", borderBottom: i < stripNews.length-1 ? "1px solid "+C.border+"33" : "none"}}>
            <div style={{flex:1, minWidth:0}}>
              <a href={n.url} target="_blank" rel="noopener noreferrer"
                style={{fontSize:11, color:C.text, textDecoration:"none", lineHeight:1.5, display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>
                {n.headline}
              </a>
              <span style={{fontSize:9, color:C.muted}}>{n.source} · {timeAgo(n.datetime)}</span>
            </div>
            <div style={{display:"flex", gap:4, flexShrink:0}}>
              {n.related && n.related.split(",").slice(0, 2).map(t => t.trim()).filter(Boolean).map(t => (
                <button key={t} onClick={() => onSelectTicker(t)}
                  style={{fontSize:8, padding:"2px 6px", background:"rgba(56,189,248,0.1)", color:C.blue, border:"1px solid "+C.blue+"22", borderRadius:4, cursor:"pointer", fontFamily:"inherit"}}>
                  {t}
                </button>
              ))}
            </div>
...
export default function App() {
  ...
  // ── Keyboard shortcuts ──
  useEffect(() => {
    const MAP = { m:"market", h:"heatmap", e:"earnings", n:"news", o:"options", t:"trade", g:"greeks", s:"scanner", w:"watchlist", p:"portfolio", a:"learn", j:"journal", r:"risk_calc", c:"crypto", b:"backtester", "?":"settings" };
    const handle = (ev) => {
  ...
  const ALL_PAGES = [
    ["market","Market"],["heatmap","🗺 Heatmap"],["earnings","📅 Earnings"],["news","📰 News"],
    ...
    <div style={{ fontFamily: "'Space Mono',monospace", background: theme==="light"?"#f0f4f8":C.bg, minHeight: "100vh", color: theme==="light"?"#0f172a":C.text, fontSize: 13, transition:"background .3s, color .3s" }}>
      ...
      {menuOpen && (
        <div className="mobile-nav" style={{ position:"fixed", inset:0, background:"rgba(6,8,15,0.98)", zIndex:300, overflowY:"auto", padding:20 }}>
          ...
          {[
            ["Markets",   [["market","📈 Market"],["heatmap","🗺 Heatmap"],["earnings","📅 Earnings"],["news","📰 News"],["crypto","₿ Crypto"]]],
      ...
      <div style={{ maxWidth: 1380, margin: "0 auto", padding: "16px 14px" }}>
        ...
        {page === "market" && (() => {
          ...
          <div>
              ...
              return (
                <div style={{ background: DATA_CONFIG.USE_LIVE_DATA ? "#071410" : "#0a1220", border:"1px solid "+(DATA_CONFIG.USE_LIVE_DATA ? C.green : C.teal)+"44", borderRadius:8, padding:"8px 14px", marginBottom:14, display:"flex", alignItems:"center", gap:10, fontSize:10, flexWrap:"wrap" }}>
                  ...
                  <div style={{ marginLeft:"auto", display:"flex", gap:8, alignItems:"center" }}>
                    ...
                    <button onClick={()=>setPage("news")} style={{ padding:"3px 10px", background:"rgba(56,189,248,0.1)", color:C.blue, border:"1px solid "+C.blue+"33", borderRadius:5, cursor:"pointer", fontSize:9, fontFamily:"inherit" }}>📰 News →</button>
                  </div>
                </div>
              );
            })()}
            ...
            {/* ── LIVE NEWS STRIP ── */}
            <MarketNewsStrip onSelectTicker={selectTicker} onViewAll={()=>setPage("news")} />
        ...
        {page === "news"       && <MarketNewsPage ticker={ticker} onSelectTicker={selectTicker} />}
